 ...
 * {@link WorksheetBuilder} to produce the new BasicWorksheet itself,
...
public final class WorksheetReader {
   ...
   * @param <T> the type of BasicWorksheet to produce
   */
   ...
   * @param builder The source of the new BasicWorksheet object
   * @param readable the input source for the contents of this BasicWorksheet
   * @param <T> the type of BasicWorksheet to produce
   * @return the fully-filled-in BasicWorksheet
   */
  public static <T> T read(WorksheetBuilder<T> builder, Readable readable) {
    ...
    while (scan.hasNext()) {
      int col;
      int row;
      // While loop skips comments.
      while (scan.hasNext("#.*")) {
        scan.nextLine();
        scan.skip("\\s*");
      }
      // Look for next Coordinate block.
      ...
      // End of look for coord. Skip the next space.
      scan.skip("\\s*");
      // While loop skips comments again.
      while (scan.hasNext("#.*")) {
        scan.nextLine();
        scan.skip("\\s*");
      }
      // Extract the next String representing contents of cell at specified Coord.
